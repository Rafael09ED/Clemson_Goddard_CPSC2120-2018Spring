...
void PropaTree::expandTreeRandom() {
    std::mt19937 rng(rd());    // random-number engine used (Mersenne-Twister in this case)
    std::uniform_int_distribution<int> uni(1, head->descendantLeaves); // guaranteed unbiased
    auto random_integer = uni(rng);
    addAtLeaf(random_integer);
}

int PropaTree::inbalance() {
    return head->inbalance();
}
...
int Node::inbalance() {
    return abs(right->descendantLeaves - left->descendantLeaves);
}
